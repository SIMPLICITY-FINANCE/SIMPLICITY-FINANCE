...
export default async function QuarterlyReportPage({ params }: { params: Promise<{ quarter: string }> }) {
  ...
  if (!/^\d{4}-Q[1-4]$/.test(quarter)) {
    return (<div className="text-center py-12"><p className="text-gray-600 text-sm">Invalid quarter format. Expected: YYYY-QN</p><a href="/reports?tab=quarterly" className="text-blue-600 hover:underline text-xs mt-2 inline-block">Back</a></div>);
  }
  ...
  if (!report) return (<div className="text-center py-12"><FileText size={40} className="mx-auto text-gray-400 mb-3" /><p className="text-gray-600 text-sm mb-1">No quarterly report for {quarter}</p><a href="/reports?tab=quarterly" className="text-blue-600 hover:underline text-xs">Back</a></div>);
  ...
  if (report.status === "failed") return (<div className="text-center py-12"><p className="text-red-600 text-sm mb-1">Failed</p><p className="text-gray-500 text-xs mb-3">{report.summary}</p><a href="/reports?tab=quarterly" className="text-blue-600 hover:underline text-xs">Back</a></div>);
    ...
    <div className="min-h-screen bg-gray-50">
      <div className="max-w-[1100px] mx-auto px-6 py-8 space-y-6">
        <div className="space-y-4">
          <a href="/reports?tab=quarterly" className="inline-flex items-center gap-1.5 text-[11px] text-gray-500 hover:text-gray-900"><ArrowLeft size={14} />Back to Reports</a>
        ...
        {content?.majorThemes && content.majorThemes.length > 0 && (
          <SC icon={<Globe size={16} className="text-gray-700" />} title="Major Themes">
            <div className="space-y-4">
                  ...
                  <div key={i} className="bg-white rounded-xl border border-gray-200 shadow-sm p-4">
                    ...
                    {t.monthByMonth && t.monthByMonth.length > 0 && (
                      <div className="space-y-1 pl-4 border-l-2 border-blue-200">
                        {t.monthByMonth.map((m, j) => (<p key={j} className="text-[11px] text-gray-600 leading-snug">{m}</p>))}
                      </div>
                    )}
                  </div>
                );
        ...
        {content?.predictions && content.predictions.length > 0 && (
          <SC icon={<Target size={16} className="text-gray-700" />} title="Predictions">
            <div className="space-y-4">
              {content.predictions.map((p: QuarterlyPrediction, i: number) => (
                <div key={i} className="bg-white rounded-xl border border-gray-200 shadow-sm p-4">
                  <p className="text-[11px] text-gray-900 leading-snug font-medium mb-2">{p.prediction}</p>
                  <p className="text-[10px] text-gray-600 mb-2">{p.basis}</p>
        ...
        {content?.sentiment && (
          <SC icon={<BarChart3 size={16} className="text-gray-700" />} title="Market Sentiment">
            <div className="bg-white rounded-xl border border-gray-200 shadow-sm p-4">
              ...
              <p className="text-[11px] text-gray-700 leading-snug mb-2">{content.sentiment.quarterNarrative}</p>
