    ...
    return (
      <div className="min-h-screen bg-background">
        <div className="max-w-[1100px] mx-auto px-6 py-8 text-center">
          <p className="text-muted-foreground text-sm">Invalid date format</p>
    ...
    return (
      <div className="min-h-screen bg-background">
        <div className="max-w-[1100px] mx-auto px-6 py-8 text-center">
          ...
          <p className="text-muted-foreground text-sm mb-1">No report for {date}</p>
          <p className="text-muted-foreground text-xs mb-3">A daily report hasn't been generated for this date yet.</p>
    ...
    return (
      <div className="min-h-screen bg-background">
        <div className="max-w-[1100px] mx-auto px-6 py-8 text-center">
          ...
          <p className="text-muted-foreground text-sm mb-1">Generating report for {date}...</p>
          <p className="text-muted-foreground text-xs">This usually takes 30-60 seconds.</p>
        </div>
      </div>
    );
  }
    ...
    <div className="min-h-screen bg-background">
      <div className="max-w-[1100px] mx-auto px-6 py-8 space-y-6">
        ...
        <div className="space-y-4">
          {/* Back link */}
          <a href="/reports" className="inline-flex items-center gap-1.5 text-[11px] text-muted-foreground hover:text-foreground transition-colors">
            <ArrowLeft size={14} />
            Back to Reports
          </a>
          ...
          {content?.themes && content.themes.length > 0 && (
            <div className="flex flex-wrap gap-1.5">
              {content.themes.map((theme, i) => (
                <span
                  key={i}
                  className="px-2.5 py-1 bg-muted text-foreground text-xs rounded-full border border-border"
                >
                  {theme.name}
                </span>
              ))}
        ...
        {content?.insights && content.insights.length > 0 && (
          <SectionCard icon={<Lightbulb size={16} className="text-foreground" />} title="Key Insights">
            <div className="space-y-4">
              {content.insights.map((insight: ReportInsight, i: number) => (
                <div key={insight.id || i} className="bg-card rounded-xl border border-border shadow-sm p-4">
                  ...
                  {/* Main insight text */}
                  <p className="text-[11px] text-foreground leading-snug mb-3">
                    {insight.text}
                  </p>
                  ...
                  {insight.evidence && insight.evidence.length > 0 && (
                    <div className="mt-3 pt-3 border-t border-border">
                      <p className="text-[10px] text-muted-foreground mb-1">Source Episodes:</p>
        ...
        {content?.sentiment && (
          <SectionCard icon={<BarChart3 size={16} className="text-foreground" />} title="Market Sentiment">
            <div className="bg-card rounded-xl border border-border shadow-sm p-4">
              ...
              <p className="text-[11px] text-muted-foreground leading-snug">{content.sentiment.reasoning}</p>
            </div>
          </SectionCard>
        )}
          ...
          <SectionCard icon={<span className="text-sm">💬</span>} title="Notable Moments">
            <div className="space-y-4">
              {content.notableMoments.map((moment: NotableMoment, i: number) => (
                <div key={i} className="bg-card rounded-xl border border-border shadow-sm p-4 flex gap-3">
                  <QuoteIcon size={16} className="text-muted-foreground mt-0.5 flex-shrink-0" />
                  <div className="flex-1 min-w-0">
                    <p className="text-[11px] text-foreground leading-snug mb-1">
                      "{moment.quote}"
                    </p>
                    ...
                    <p className="text-[10px] text-muted-foreground mt-1.5 bg-muted rounded px-2 py-1 inline-block border border-border">
                      {moment.whyNotable}
                    </p>
                  </div>
                </div>
                ...
                <div key={ep.episode_id} className="flex items-center justify-between py-2 px-3 rounded-lg hover:bg-muted transition-colors">
                  <div className="flex-1 min-w-0">
                    <a
                      ...
                      className="text-[11px] font-medium text-foreground hover:text-blue-600 transition-colors"
                    >
                      {ep.title}
                    </a>
                  ...
                  {ep.video_id && (
                    <a
                      ...
                      className="text-[10px] text-muted-foreground hover:text-blue-600 ml-3 flex-shrink-0"
                    >
                      YouTube →
                    </a>
                  )}
